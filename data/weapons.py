...
class Projectile(pygame.sprite.Sprite):
    ...
    ACCELERATION = -0.05
    SIZE = (6, 25)
    IMAGE_PATH = pygame.image.load(os.path.join('images', 'laser_enemy.png'))
    LAUNCH_VOLUME = 0.03
    ...
    def __init__(self, position, settings):
        ...
        for i in range(self.NUMBER_OF_FRAMES):
            missile = pygame.image.load(
                os.path.join('images', 'rocket', f'rocket_3_Flying_00{i}.png')).convert_alpha()
            self.frames.append(pygame.transform.scale(missile, self.SIZE))
        self.current_frame = 0
        self.image = self.frames[self.current_frame]
        self.rect = self.image.get_rect(center=position)
        self.speed = self.START_SPEED
        self.sound_effect = pygame.mixer.Sound(os.path.join('audio', 'rocket_launch.mp3'))
...
class Explosion(pygame.sprite.Sprite):
    ...
    def __init__(self, position):
        ...
        self.frames = []
        for i in range(self.NUMBER_OF_FRAMES):
            explosion = pygame.image.load(os.path.join('images', 'rocket', f'rocket_3_Explosion_00{i}.png'))
            self.frames.append(pygame.transform.scale(explosion, (self.SIZE)))
        self.current_frame = 0
        self.image = self.frames[self.current_frame]
        self.rect = self.image.get_rect(center=position)
        self.sound_effect = pygame.mixer.Sound(os.path.join('audio', 'rocket_explosion.mp3'))
...
class Beam(pygame.sprite.Sprite):
    BEAM_DURATION = 1200
    BEAM_SIZE = (125, 800)
    BEAM_IMAGE = pygame.image.load(os.path.join('images', 'laser_beam.png'))
    BEAM_VOLUME = 0.1
