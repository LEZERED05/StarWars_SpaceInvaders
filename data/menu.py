...
class Button(pygame.sprite.Sprite):
    ...
    def __init__(self, text, position, screen):
        ...
        self.text = text
        self.text_font = pygame.font.Font(os.path.join('images', 'Pixeltype.ttf'), self.FONT_SIZE)
...
class Menu:
    ...
    FONT_SIZE = 36
    BACKGROUND_IMAGE = pygame.image.load(os.path.join('images', 'background_menu.jpg'))
    ...
    def __init__(self, screen, SETTINGS, message):
        ...
        self.screen = screen
        self.button_run = Button(text='RUN', position=(0.5 * self.SETTINGS['WIDTH'], 0.35 * self.SETTINGS['HEIGHT']),
                                 screen=screen)
        self.button_exit = Button(text='EXIT', position=(0.5 * self.SETTINGS['WIDTH'], 0.65 * self.SETTINGS['HEIGHT']),
                                  screen=screen)
        self.button_settings = Button(text='SETTINGS',
                                      position=(0.5 * self.SETTINGS['WIDTH'], 0.5 * self.SETTINGS['HEIGHT']),
                                      screen=screen)
